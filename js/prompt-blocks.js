 ...
 *   {{resource}}    → Q5: Pasted resource text
 *   {{language}}    → EAL/D: Other language field
 *   {{direction}}   → Injected by assembler based on Q2 (support/extension/both)
 *
 * Usage:
 *   const block = PB.buildBlock('readability', ctx, direction);
 *   // Returns a fully assembled, paste-ready prompt string.
 */

const PB = (() => {

  // ─── CONTEXT INJECTION ────────────────────────────────────────────────────
  // Replaces manual placeholder patterns with actual teacher-supplied context.
  ...
  function inject(template, ctx, extras) {
    ...
    const resource = ctx.resource || '[paste your resource here]';

    return template
      .replace(/\[paste text\]/gi, resource)
      .replace(/\[paste text or describe content\]/gi, resource)
      .replace(/\[paste content\]/gi, resource)
      .replace(/\[paste instructions\]/gi, resource)
      .replace(/\[describe task\]/gi, resource)
      .replace(/\[paste full task description\]/gi, resource)
      .replace(/\[Insert content\/text\/lesson description\]/gi, resource)
      .replace(/\[describe activity\]/gi, resource)
      .replace(/\[grade level\]/gi, year)
      .replace(/\[Year Level\]/g, year)
      .replace(/\[grade\/subject\]/gi, `${year} ${subject}`)
      ...
      .replace(/\[number\] EALD students/gi, 'EALD students')
      .replace(/\[Key idea 1\]/gi, '(key idea 1 — drawn from the resource above)')
      .replace(/\[Key idea 2\]/gi, '(key idea 2 — drawn from the resource above)')
      .replace(/\[Key idea 3\]/gi, '(key idea 3 — drawn from the resource above)')
      .replace(/\[concept\]/gi, subject)
      .replace(/\[list terms\]/gi, '(identify from the resource above)')
      .replace(/\[list terms with definitions\]/gi, '(identify from the resource above)')
      .replace(/\[describe thinking\/writing task\]/gi, resource);
  }

  // ─── DIRECTION WRAPPERS ───────────────────────────────────────────────────
  // These frame the prompt with pedagogical clarity about support vs extension.

  const DIRECTION = {
    support: `This differentiation is for students who need additional support and scaffolding. The goal is to reduce barriers to access while maintaining the intellectual integrity of the learning. Do not simplify the ideas — make them more reachable.`,

    extension: `This differentiation is for students who are ready for greater depth and challenge. The goal is to increase complexity, abstraction, and autonomy — not to add volume. Push the thinking, not the workload.`,

    both: `Produce two clearly labelled versions:
— Support version: for students who need additional scaffolding and access
— Extension version: for students ready for greater depth and challenge
Both versions must address the same learning intention. Vary the complexity and support, not the learning goal.`
  };

  // ─── SOURCE PROMPTS ───────────────────────────────────────────────────────
  // Verbatim from the manual. Placeholders injected at runtime.
  ...
  const SOURCE = {
    ...
    readability: {
      ...
      template: `Here is a text I'm using with my {{year}} class:

{{resource}}

Create versions of this text as follows:
- Support version: Simplified vocabulary and sentence structure for students reading below grade level, but maintain the conceptual integrity. Do not "dumb down" the ideas — make them more accessible.
...
Ensure all versions convey the same core concepts and maintain similar length (±20%).`
    ...
    conceptComplexity: {
      ...
      template: `I am teaching {{subject}} to {{year}} students.

Here is the resource:

{{resource}}

The learning intention is: {{intention}}

Step 1 — Create two versions of this resource:
...
- Abstract version: Focuses on themes, implications, underlying principles. Uses "Why" and "what does this mean" questions. Asks students to work with generalisations and transferable ideas.

Both versions must address the same core content but at different levels of abstraction.

Step 2 — For the more abstract version, generate 3 analogies that make the key concept more approachable for students who find it hard to visualise. For each analogy:
- Explain it in 2–3 sentences, as you'd say it to a student
- Map the key elements: "In this analogy, [X] represents [Y]"
- Note where the analogy breaks down, so I don't accidentally create a misconception`
    },

    length: {
      // Source: C — "Identify Key Passages"
      source: 'step3-content.html — Highlighted Print Materials',
      template: `Here is a text my {{year}} students need to read:

{{resource}}

Help me identify:
- Which paragraphs or passages contain the essential information aligned to this learning intention: {{intention}}
- Which sections are supplementary or provide examples but aren't critical
- A suggested highlighting or reading strategy for students who need support (e.g., "Read highlighted sections first, then return to read the full text if time permits")
...
Then produce a shortened version containing only the essential passages, with brief bridging sentences where needed to maintain coherence.`
    ...
    backgroundKnowledge: {
      ...
      template: `Students will be reading the following text:
...
Create a one-page digest that students can use as a reference before and while they read. Include:
...
Format it clearly so students can use it as a reference while they read.`
    ...
    visualSupports: {
      ...
      template: `Context: I have a resource that is primarily text-based, and I need to make it more accessible for students who process information better with visual supports.
Objective: Redesign the resource as a multimodal version using text, simple visuals, and colour coding.
Instructional nuance: Use colour and visuals purposefully, not decoratively. Each element should support comprehension, not just make it look appealing.
...
Please redesign it as a multimodal version:
- Text: Keep key instructions and content but make them concise and concrete
- Visuals: Suggest simple icons, diagrams, or symbols to represent key steps or concepts — describe each clearly enough that I can create or find them
- Colour coding: Suggest a colour scheme to help students distinguish between types of information (e.g., instructions vs. examples), see sequence or structure, and identify what's most important
...
- Colour coding rationale (e.g., "Blue = instructions, Green = examples")`
    ...
    cognitiveDemand: {
      ...
      template: `Here is a set of questions / task:
...
Create versions of these questions that adjust the cognitive demand using Bloom's Taxonomy:
...
Ensure all versions require genuine thinking — the support version isn't "easier," it's addressing a different level of cognitive complexity. Do not remove intellectual demand from the support version; reduce abstraction instead.`
    },
    ...
    questionScaffolding: {
      ...
      template: `Here are the questions / task I'm asking students to complete:
...
Use student-friendly language for {{year}}.`
    },

    studentChoice: {
      // Source: E — "Adjust Level of Structure"
      source: 'step5-equalizer.html — Structured ↔ Open-Ended',
      template: `Here is the task / question set I'm giving students:
...
Create two versions that adjust the level of student autonomy:
...
Both versions must result in equivalent evidence of learning against this learning intention. Vary the independence, not the intellectual standard.`
    },

    chunking: {
      // Source: C — "Rewrite with Chunked Steps and Concrete Language"
      source: 'step3-content.html — Neurodiversity-Responsive Content Design',
      template: `Context: My questions or task instructions may be difficult for students who need concrete, sequential information — either because the language is abstract or because multiple demands are packaged together.
...
Please rewrite the questions / instructions as:
- Numbered steps (no more than 8 if possible)
- Concrete, specific language — avoid metaphors, abstract terms, or implied information
- Active verbs that tell students exactly what to do
- One clear action per step

If any step is still complex, break it into sub-steps (e.g., 3a, 3b).

Also note any abstract language you replaced and explain why the original phrasing might have been confusing.`
    },
...
Redesign the questions to offer students a genuine choice in how they demonstrate their understanding. Provide at least three response mode options:

1. Written response: The standard written answer — but redesign the question so it genuinely requires students to think, not just transcribe.
2. Visual response: A diagram, annotated image, concept map, flowchart, or other visual that demonstrates understanding — specify exactly what the visual should show and how it should be labelled.
...
For each mode: write the question or task instruction as the student would receive it. Ensure each mode genuinely assesses the same learning intention — the mode changes, not the cognitive demand.`
    },
    ...
    ealdGlossary: {
      ...
      template: `Context: I'm teaching {{subject}} to students in {{year}}.
Objective: Identify and define key academic vocabulary that students will need to understand this content.
Instructional nuance: Focus on Tier 2 (high-utility academic words like "analyse," "synthesise") and Tier 3 (domain-specific terms). Provide student-friendly definitions that are age-appropriate and avoid circular language.

Here is the content I'm teaching:
...
Format the output as a table with columns: Term | Definition | Example sentence | Tier`
    },
    ...
    ealdSubjectVocab: {
      ...
      template: `Context: I'm teaching {{subject}} to students in {{year}}. My EALD students need explicit support with the subject-specific language of this discipline — not just vocabulary definitions, but understanding how subject-specific language works and what it expects of them.
Objective: Identify and unpack the subject-specific vocabulary and language patterns in this content.
Instructional nuance: Subject disciplines each have their own way of using language. In geography, "explain the relationship between..." expects a particular kind of response. In science, "hypothesis" has a precise meaning unlike everyday usage. Surface these discipline-specific language demands explicitly.
...
Please:
1. Identify 6–10 subject-specific terms that carry precise meaning in {{subject}} — terms that either differ from everyday usage or that EALD students are unlikely to encounter outside this subject
2. For each term: provide a student-friendly definition and note how the meaning differs from everyday English if relevant
...
4. Format as a reference sheet students can keep beside the resource`
    ...
    ealdSentenceFrames: {
      ...
      template: `Students need to respond to the following task:
...
Create sentence starters and frames specifically designed to support EALD students in constructing academic written responses to this task. Include frames that help students:
...
Also provide a brief note for the teacher on which frames are most useful for early-stage vs. developing EALD learners.`
    },

    ealdVisualSupports: {
      // Source: S — "Add Visual Supports to Vocabulary"
      source: 'step6-scaffolding.html — Add Visual Supports to Vocabulary',
      template: `Context: I have key vocabulary from this resource that would be more accessible if students had visual cues alongside the definitions.
Objective: Suggest simple icons, symbols, or visual representations that would support vocabulary learning for EALD students.
Instructional nuance: Visuals should be simple, unambiguous, and culturally appropriate. They are memory aids and comprehension supports — not artwork.
...
For the key vocabulary in this resource, suggest:
...
Prioritise visuals that are simple to draw or find, unambiguous across cultures, and memorable.`
    ...
    ealdSyntax: {
      // Source: C — "Create Tiered Versions" with EAL/D framing added
      source: 'step3-content.html — Tiered Texts (EAL/D syntax variant)',
      template: `Context: I have EALD students who can engage with the ideas in this text but are held back by complex sentence structures, embedded clauses, and dense academic syntax.
...
Instructional nuance: This is not about simplifying the ideas. The concepts should remain intact. Simplify the sentence structures: shorten sentences, unpack embedded clauses, use active rather than passive voice, and make implicit logical connections explicit.
...
Please produce a simplified syntax version that:
...
After the revised text, provide a brief note on the main structural changes you made.`
    ...
    ealdMandarin: {
      ...
      template: `Context: I have EALD students whose home language is Mandarin who need vocabulary support to access content-heavy material.
...
Instructional nuance: This is a scaffold, not a replacement for English instruction. The Mandarin column provides a cognitive anchor so students can focus on concept learning rather than decoding every term. Include student-friendly definitions in both languages — not just direct word translations, which often miss nuance.

I'm teaching {{subject}} to {{year}} students. Here is the content:

{{resource}}

Please create a bilingual English–Mandarin glossary with:
...
Important: If a direct translation doesn't capture the meaning well, note this and provide a brief explanation rather than a misleading literal translation.`
    ...
    ealdOtherLanguage: {
      ...
      template: `Context: I have EALD students whose home language is {{language}} who need both vocabulary support and clarity on task instructions.
...
I'm teaching {{subject}} to {{year}} students. Here is the content / task:
...
- Format with English on the left and {{language}} on the right`
    ...
    neuroChunked: {
      // Source: C — "Rewrite with Chunked Steps and Concrete Language"
      source: 'step3-content.html — Neurodiversity-Responsive Content Design',
      template: `Context: My resource contains instructions or content that may be difficult for students who need concrete, sequential information — either because steps are bundled together, the language is abstract, or expectations are implied rather than stated.
Objective: Rewrite as numbered steps using clear, concrete language.
Instructional nuance: Replace abstract concepts with concrete examples. Use active verbs. Make implicit steps explicit. One clear action per step.
...
Please rewrite the instructions and/or questions as:
- Numbered steps (no more than 8 if possible)
...
Note any abstract or implicit language you changed and explain why the original phrasing may have been confusing for students who need explicit instruction.`
    ...
    neuroWorkingMemory: {
      ...
      source: 'step3-content.html — Neurodiversity-Responsive Content Design',
      template: `Context: I have students with working memory challenges who struggle to hold multiple pieces of information in mind while completing tasks.
Objective: Design supports that reduce working memory demands while affirming student strengths.
Instructional nuance: Frame supports as tools for efficiency, not as accommodations for deficiency. Use a strengths-first lens.
...
First, identify what this task requires students to hold in working memory simultaneously (e.g., remember the question while reading the text, track multiple steps, recall prior knowledge while generating a new response).
...
Suitable tools might include: reference sheets, colour-coded reminders, graphic organisers, partially completed frames, read-aloud cues, or chunked check-in points.`
    ...
    neuroWhiteSpace: {
      ...
      template: `Context: I need to redesign this resource so it is easier to navigate for students who experience visual overwhelm, have difficulty organising information on a page, or benefit from a cleaner, less cluttered layout.
...
3. Any content that could be moved to a separate reference sheet to reduce the visual load of the main task`
    ...
    neuroExplicitProcess: {
      ...
      template: `Here is the task I'm asking students to complete:
...
Create an explicit process guide for this task — a step-by-step procedure that students can follow independently. The guide should:
...
Use student-friendly language appropriate for {{year}}. The goal is that a student who is unsure what to do next can consult this guide and get unstuck without needing teacher input.`
    ...
    neuroReducedOutput: {
      ...
      template: `Context: Some students are held back not by the thinking required but by the output required — extended written responses create a barrier that masks genuine understanding.
Objective: Redesign this task to offer output alternatives that reduce the written production demand while maintaining the cognitive demand.
Instructional nuance: The goal is to assess understanding, not writing stamina. Alternative outputs should require equivalent thinking, just expressed differently.
...
Redesign the task to offer three output alternatives that each demonstrate the same understanding:
...
3. Structured dot-point response: A highly scaffolded written option with clear instructions: how many points, what each point should cover, with sentence starters for each.
...
For each option: write it as the student would receive it, clearly enough that they can complete it independently.`
    ...
    neuroTTS: {
      ...
      template: `Context: Some students use text-to-speech software to access written resources. Formatting choices that work well visually can create confusion when read aloud — tables become unreadable, abbreviations are mispronounced, and text embedded in images is invisible to the software.
Objective: Reformat this resource so it works well when read by text-to-speech software, without changing the content.
Instructional nuance: Text-to-speech reads linearly. It cannot interpret layout, visual hierarchy, or spatial relationships. The resource needs to communicate structure through language, not formatting.
...
5. Flag any content that is currently embedded in images and cannot be read by TTS — suggest how to provide equivalent text-based access
6. Note any other TTS compatibility issues you identify`
    ...
    neuroAnchor: {
      ...
      template: `I need anchor activities for students who finish the main task early, so they have something meaningful to move to independently — without needing new instructions from me.
...
Create a menu of 4–5 anchor activities for this context. Each should:
...
- Accommodate a range of readiness levels (a student who finished quickly and a student who needed extra time to finish should both find something appropriate)
- Be sustainable — usable across multiple lessons in this topic

For each anchor activity:
...
- Note the approximate time required`
  ...
  // ─── DIRECTION-AWARE PROMPT SELECTORS ────────────────────────────────────
  // Some prompts need direction-specific phrasing. These handle that.

  function directionClause(direction, subject) {
    if (direction === 'support') return `This is for students who need additional support in ${subject || 'this subject'}. Prioritise access, scaffolding, and reducing barriers without reducing intellectual demand.`;
    if (direction === 'extension') return `This is for students who are ready for greater depth and challenge in ${subject || 'this subject'}. Prioritise complexity, abstraction, and autonomy.`;
    if (direction === 'both') return `Produce clearly labelled support and extension versions. Both must address the same learning intention — vary the complexity and scaffolding, not the learning goal.`;
    return '';
  }
   ...
   * @returns {string} Fully assembled prompt ready to copy into an LLM
   */
  function buildBlock(key, ctx, direction, extras) {
    ...
    const dirClause = directionClause(direction, ctx.subject);

    // Prepend direction clause as a clearly labelled framing line
    const lines = [
      dirClause ? `Differentiation direction: ${dirClause}` : null,
      assembled
    ].filter(Boolean);
